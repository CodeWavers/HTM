...
// Insert Countra voucher 
     public function insert_contravoucher() {
        ...
        $IsPosted = 1;
        $IsAppove = 0;
        $CreateBy = $this->session->userdata('user_id',TRUE);
        $createdate = date('Y-m-d H:i:s');
        for ($i = 0; $i < count($cAID); $i++) {
            $contrainsert = array(
                'VNo' => $voucher_no,
    ...
    public function trial_balance_report($FromDate,$ToDate,$WithOpening){
     ...
     //al hassan working
      public  function get_vouchar(){
         ...
         $date=date('Y-m-d');
          $sql="SELECT VNo, Vtype,VDate,IsAppove, SUM(Debit+Credit)/2 as Amount FROM acc_transaction  WHERE VDate='$date' AND VType IN ('DV','JV','CV') GROUP BY VNO, Vtype, VDate ORDER BY VDate";
